...
class Shape
  ...
  constructor: (_adapter, _type, _obj, _settings) ->
    @.adapter    = _adapter
    @.settings   = _settings
    @.serializer = (new Serializer _settings)
  ...
  defaultData: ->
    ...
    fs = (map @.settings.fields, (_f) =>
      return if @.settings.group then "meta.#{_f}" else _f
    )
    return (pick df, fs)
  ...
  defaultPaths: ->
    return (map @.settings.fields, (_f) =>
      return if @.settings.group then "meta.#{_f}" else _f
    )


  extend: (_type, _obj, _settings = {}) ->
    d = { data: (omit (cloneDeep @.data), @.defaultPaths()) }
    v = { validators: (omit (cloneDeep @.validators), @.defaultPaths()) }
    obj = (merge d, v, _obj)
    settings = (@.settings.extend _settings)
    return (new Shape @.adapter, _type, obj, settings)
  ...
  save: (_data) ->
    ...
    collection = if @.settings.group then obj.meta.collection else obj.collection
    id = if @.settings.group then obj.meta.id else obj.id
  ...
  update: (_data, _fields) ->
    ...
    prefix = if @.settings.group then 'meta.' else ''

    ca_path = prefix + (@.serializer.fieldCasing 'created_at')
    if (includes @.settings.fields, 'created_at') && !(includes _fields, ca_path)
      (unset updates, ca_path)

    ua_path = prefix + (@.serializer.fieldCasing 'updated_at')
    if (includes @.settings.fields, 'updated_at') && !(includes _fields, ua_path)
