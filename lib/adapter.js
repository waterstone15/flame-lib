      ...
      page(_opts, _collection, _shape) {
        ...
        readable = {
          ...
          read: async() => {
            ...
            [col_first, col_last, items, next, prev] = (await all([this.find(_collection, col_fstQ, _shape, fields).read(), this.find(_collection, col_lstQ, _shape, fields).read(), this.list(_collection, itemsQ, _shape).read(), this.find(_collection, nextQ, _shape, fields).read(), this.find(_collection, prevQ, _shape, fields).read()]));
            prev = (function() {
              switch (false) {
                case !(cursor && field && !at_end):
                  return prev;
                case !(at_end && items.length > size):
                  return nth(items, -1);
                default:
                  return null;
              }
