(function() {
  var DateTime, FlameError, Serializer, Shape, Spark, cloneDeep, every, forEach, get, includes, intersection, isArray, isBoolean, isEmpty, isEqual, isFunction, isString, map, merge, omit, pick, random, set;
  ...
  Shape = class Shape {
    ...
    update(_data, _fields) {
      var collapsed, collection, id, now, obj, path, updates, writable;
      if (!this.ok(_data, _fields) || _fields === null) {
        return null;
      }
      obj = this.obj(_data);
      updates = this.obj(_data, _fields);
      if (includes(this.config.fields, 'updated_at')) {
        now = DateTime.local().setZone('utc').toISO();
        path = (this.config.group ? 'meta.' : '') + this.serializer.fieldCasing('updated_at');
